...
GetParticipantsHandler::GetParticipantsHandler(const userver::components::ComponentConfig& config,
...
std::string GetParticipantsHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    if (request.GetMethod() != userver::server::http::HttpMethod::kPost) {
        ...
        return userver::formats::json::ToString(userver::formats::json::MakeObject(
            "error", "Method not allowed. Use POST to get participants."
        ));
    }
    ...
    try {
        ...
        GetParticipantsRequest participants_request;
        participants_request.event_id = request_body["event_id"].As<std::string>("");

        if (participants_request.event_id.empty()) {
        ...
        // Apply rate limiting using event_id as identifier
        std::string rate_limit_key = "get-participants:" + participants_request.event_id;
        auto rate_limit_info = rate_limiter_.IsAllowed(rate_limit_key, "get-participants", kRateLimitMaxRequests, kRateLimitWindowSeconds);
        ...
        const std::string cache_key = participants_request.GetCacheKey();
        ...
        filter.event_id = participants_request.event_id;
