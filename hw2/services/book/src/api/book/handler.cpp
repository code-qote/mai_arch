...
std::string BookEventHandler::HandleRequest(
    ...
    try {
        ...
        auto booking_result = common::database::InMemoryDb::Instance().CreateBooking(book_request.event_id, user_id, book_request.places_count);
        ...
        if (!ReservePlacesInEventService(book_request.event_id, book_request.places_count, auth_token)) {
            common::database::InMemoryDb::Instance().UpdateBookingStatus(booking.id, common::database::BookingStatus::kFailed);
            request.SetResponseStatus(userver::server::http::HttpStatus::kBadRequest);
            return userver::formats::json::ToString(userver::formats::json::MakeObject(
        ...
        auto update_result = common::database::InMemoryDb::Instance().UpdateBookingStatus(booking.id, common::database::BookingStatus::kConfirmed);
