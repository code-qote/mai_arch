...
SearchResponse ProcessSearch(
    const SearchRequest& request,
    common::database::InMemoryDb& db) {
    
    SearchResponse response;
    response.users = db.FindUsers(request.ToFilter());
...
std::string SearchHandler::HandleRequest(
    ...
    try {
        ...
        auto response = ProcessSearch(search_request, common::database::InMemoryDb::Instance());
